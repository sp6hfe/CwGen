...
def load(dictionary_path):
    ...
    with open(os.path.normpath(dictionary_path), mode="r", encoding="ISO-8859-1") as dictionary:
        for line in dictionary:
            ...
            if data_count == 1:
                loaded_dictionary.setdefault(len(split_data[0]), []).append([split_data[0], 'None', 'None'])
            # handle more complex dictionary having additional data like ispell [word/metadata occurence]
                ...
                if wm_list_len == 1:
                    loaded_dictionary.setdefault(len(wm_list[0]), []).append([wm_list[0], 'None', split_data[1]])
                elif wm_list_len >= 2:
                    loaded_dictionary.setdefault(len(wm_list[0]), []).append([wm_list[0], wm_list[1], split_data[1]])
...
def main():
    ...
    dict_path = os.path.join(os.path.dirname(script_path), dict_dir_name, test_dict)

    words_stat = get_stat(dict_path)
    print(words_stat)
